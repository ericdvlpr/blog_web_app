  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 mt-10 px-10 md:px-15 lg:px-32 gap-8">
      {posts?.map((item, index) => {
        return (
          <div key={index} className="m-4 cursor-pointer" onClick={()=>navigate()}>
            <img
              src={item.img}
              className="w-full rounded-2xl object-cover h-[200px]"
            />
            <h3 className="text-red-500 mt-3">{item.tag}</h3>
           <a href={item.article_url}  className="text-black" target="_blank"><h2 className="font-bold mt-3">{item.title}</h2></a>
